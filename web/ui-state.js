 ...
 * @property {boolean} text_viewable
 ...
 * @property {HTMLDivElement} viewerPage
 ...
 * @property {HTMLPreElement} textViewer
 * @property {HTMLDivElement} viewerStatus
...
export function parseIndexData(value) {
  ...
  if (!["Index", "View"].includes(kind)) throw new TypeError("Invalid page kind");
  ...
  return /** @type {IndexData} */ ({
    ...
    allow_archive: false,
    dir_exists: false,
    text_viewable: requireBoolean(value, "text_viewable"),
    truncated: false,
...
function collectDom() {
  return {
    ...
    viewerPage: requireElement(".viewer-page", HTMLDivElement),
    ...
    textViewer: requireElement(".text-viewer", HTMLPreElement),
    viewerStatus: requireElement(".viewer-status", HTMLDivElement),
 ...
 */
export function directoryQueryParams(search) {
  const query = new URLSearchParams(search);
  /** @type {Record<string, string>} */
  const safeParams = {};
  // Only directory browsing state may survive into links generated by the UI.
  // Authentication material and retired feature flags must never be reflected.
  for (const name of ["q", "sort", "order"]) {
    const value = query.get(name);
    if (value !== null) safeParams[name] = value;
  }
...
export function loadAppContext() {
  ...
  const params = directoryQueryParams(window.location.search);
  return {
    data,
    params,
    dom: collectDom(),
