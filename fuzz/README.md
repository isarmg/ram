...
`cargo-fuzz 0.13.1` does not expose Cargo's `--locked` flag on `fuzz run`.
...
cargo +nightly-2026-07-20 fuzz run uri_path target/fuzz-corpus/uri_path -- \
...
`cargo-fuzz 0.13.1` 的 `fuzz run` 不暴露 Cargo `--locked`；前面的锁定 `cargo check` 会在提交图
