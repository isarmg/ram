...
export default defineConfig({
  ...
  use: {
    baseURL,
    // 中文：APIRequestContext 不共享浏览器认证缓存，API 检查需预先发送本地 Basic 凭据。
    // English: APIRequestContext does not share a browser's auth cache, so API checks send local Basic credentials preemptively.
  ...
  webServer: {
    command: 'cargo run --locked -- "$RAM_E2E_DATA_DIR" --port "$RAM_E2E_PORT" --auth admin:admin@/:rw --auth reader:reader@/:ro,/dir1:rw --allow-all --storage-reserve 0',
