...
`--all-features` 会包含仅供 harness 使用的 `fuzzing` feature；覆盖率 job 则只合并默认 TLS 与
`--no-default-features` 两种生产配置。新增 fuzz target 时按 `fuzz/README.md` 加入最小 corpus，
本地至少执行有界 smoke。
...
- 贡献默认按仓库的 `MIT OR Apache-2.0` 双许可证提供。
...
[仓库治理检查表](docs/REPOSITORY_GOVERNANCE.md) 配置 branch/tag/release 保护，并定期审计；
...
`--all-features` includes the harness-only `fuzzing` feature. Coverage combines the default TLS and
`--no-default-features` production configurations. New fuzz targets need a minimal corpus documented
in `fuzz/README.md` and at least a bounded local smoke run.
...
- Contributions are provided under the repository's `MIT OR Apache-2.0` dual license.
...
[repository governance checklist](docs/REPOSITORY_GOVERNANCE.md); forks and mirrors do not inherit it.
