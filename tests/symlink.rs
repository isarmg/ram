...
#[rstest]
fn render_index_symlink_escape_blocked(
    #[with(&["--render-index"])] server: TestServer,
    tmpdir: TempDir,
) -> Result<(), Error> {
    // index.html 本身是越出服务根的符号链接；请求目录不得跟随它服务根外秘密文件。
    // index.html escapes the served root; requesting the directory must not follow it to an outside secret.
    let secret = tmpdir.path().join("secret.txt");
    fs::write(&secret, "top secret").unwrap();
    fs::remove_file(server.path().join("index.html")).unwrap();
    symlink_dir(&secret, server.path().join("index.html")).expect("Couldn't create symlink");
    let resp = reqwest::blocking::get(server.url())?;
    assert_eq!(resp.status(), 404);
    Ok(())
}
...
#[rstest]
fn render_index_internal_symlink_allowed(
    #[with(&["--render-index", "--allow-symlink"])] server: TestServer,
) -> Result<(), Error> {
    let internal_index = server.path().join("dir1/index.html");
    fs::remove_file(server.path().join("index.html")).unwrap();
    symlink_dir(&internal_index, server.path().join("index.html"))?;
    let resp = reqwest::blocking::get(server.url())?;
    assert_eq!(resp.status(), 200);
...
#[rstest]
fn render_spa_symlink_escape_blocked(
    #[with(&["--render-spa"])] server: TestServer,
    tmpdir: TempDir,
) -> Result<(), Error> {
    // SPA 回退的 index.html 也是越出根的符号链接。
    // The SPA fallback's index.html is also a symlink escaping the root.
    let secret = tmpdir.path().join("secret.txt");
    fs::write(&secret, "top secret").unwrap();
    fs::remove_file(server.path().join("index.html")).unwrap();
    symlink_dir(&secret, server.path().join("index.html")).expect("Couldn't create symlink");
    let resp = reqwest::blocking::get(format!("{}some/missing/route", server.url()))?;
    assert_eq!(resp.status(), 404);
    Ok(())
}

#[rstest]
fn render_spa_internal_symlink_allowed(
    #[with(&["--render-spa", "--allow-symlink"])] server: TestServer,
) -> Result<(), Error> {
    let internal_index = server.path().join("dir1/index.html");
    fs::remove_file(server.path().join("index.html")).unwrap();
    symlink_dir(&internal_index, server.path().join("index.html"))?;
...
/// Destination 经符号链接越出根的 COPY 必须在根外创建任何目录之前拒绝。此回归测试针对
/// `ensure_path_parent` 曾先于包含性检查运行的问题。
/// A COPY whose Destination escapes through a symlink must be rejected before creating any outside
/// directory. This regression covers the former ordering where `ensure_path_parent` ran before the
/// containment check.
#[rstest]
fn copy_dest_symlink_escape_creates_nothing(
    // 启用上传/删除但不允许符号链接，包含性守卫保持生效（`-A` 也会开启 allow-symlink）。
    // Upload/delete are enabled but symlinks are not, so containment remains active (`-A` would also
    // enable allow-symlink).
    #[with(&["--allow-upload", "--allow-delete"])] server: TestServer,
    tmpdir: TempDir,
) -> Result<(), Error> {
    // 根内 `escape` 是指向外部目录的符号链接。 / Root-local `escape` points to an outside directory.
    let outside = tmpdir.path().join("outside");
    fs::create_dir_all(&outside).unwrap();
    symlink_dir(&outside, server.path().join("escape")).expect("Couldn't create symlink");

    // Destination 经链接解析到 `outside/newdir/copied.html`。
    // Destination resolves through the symlink to `outside/newdir/copied.html`.
    let dest = format!("{}escape/newdir/copied.html", server.url());
    let resp = fetch!(b"COPY", format!("{}test.html", server.url()))
        .header("Destination", &dest)
        .send()?;
    assert_eq!(resp.status(), 400);

    // 被拒请求不得在根外创建父目录。 / The rejected request must not create the parent outside root.
    assert!(
        !outside.join("newdir").exists(),
        "rejected COPY leaked a directory outside the served root"
    );
    Ok(())
}

#[rstest]
fn copy_dest_escape_is_bad_request_when_symlink_support_is_enabled(
    #[with(&["--allow-upload", "--allow-delete", "--allow-symlink"])] server: TestServer,
    tmpdir: TempDir,
) -> Result<(), Error> {
    let outside = tmpdir.path().join("outside-allowed-symlink");
    fs::create_dir_all(&outside)?;
    symlink_dir(&outside, server.path().join("escape-enabled"))?;

    let destination = format!("{}escape-enabled/newdir/copied.html", server.url());
    ...
    assert!(
        !outside.join("newdir").exists(),
        "rejected COPY created an ancestor outside the served capability"
    );
    Ok(())
}
