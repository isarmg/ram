...
fn spawn_writable_server(root: &Path, port: u16) -> ServerProc {
    ServerProc::spawn(writable_server_command(root, port))
}

fn writable_server_command(root: &Path, port: u16) -> std::process::Command {
...
fn committed_http_mutations_survive_sigkill_and_restart(port: u16) -> Result<(), Error> {
    ...
    let mut server = ServerProc::spawn_crash_target(writable_server_command(root.path(), port));
    ...
    drop(client);
    // 强制终止并等待崩溃进程回收。这证明进程崩溃恢复/持久化连接，不模拟断电或
    // 谎报 flush 的存储设备。
    // Force-terminate and reap the crashed process. This proves process-crash recovery and
    // durability wiring, not power loss or a storage device that lies about flush.
    server.sigkill_and_wait();
