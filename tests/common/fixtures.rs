...
/// - **`Drop` 收尸**：先 SIGTERM 并有界等待，失常时再 SIGKILL。测试中途
///   `?`/断言失败提前返回时也不会留下孤儿服务器占着端口，同时允许覆盖率运行时
///   在正常退出时完整写出 profile。
///
...
/// - **`Drop` reaping**: send SIGTERM and wait for a bounded interval, then use SIGKILL only for a
///   stuck process. This prevents an orphan from retaining the port when `?` or an assertion exits
///   early, while allowing the coverage runtime to finish writing its profile on a normal exit.
...
impl ServerProc {
    ...
    /// 启动一个将被测试故意强制终止的服务器，并禁止该子进程写 LLVM profile。
    ///
    /// 非连续覆盖率模式只在进程正常退出时保证 profile 完整；SIGKILL 可能留下
    /// llvm-profdata 无法合并的截断文件。测试夹具和重启后的服务器仍会被覆盖。
    ///
    /// Start a server that the test will deliberately terminate abruptly, and prevent that child
    /// from writing an LLVM profile.
    ///
    /// Non-continuous coverage guarantees a complete profile only on normal process exit; SIGKILL
    /// can leave a truncated file that llvm-profdata cannot merge. The harness and restarted server
    /// remain covered.
    pub fn spawn_crash_target(mut cmd: Command) -> Self {
    ...
    /// 立即强制终止并回收服务器，仅供明确的进程崩溃测试使用。
    /// Force-terminate and reap the server; use only in explicit process-crash tests.
    pub fn sigkill_and_wait(&mut self) {
...
impl Drop for ServerProc {
    fn drop(&mut self) {
        ...
        // 普通测试应模拟服务管理器的正常停止，使服务器完成清理，也让 LLVM 覆盖率
        // 运行时完整写出 profile。若子进程失常，则有界等待后仍以 SIGKILL 保证收尸。
        // Ordinary teardown models a service-manager stop so the server can clean up and the LLVM
        // coverage runtime can finish its profile. A bounded wait followed by SIGKILL still
        // guarantees reaping if the child is stuck.
