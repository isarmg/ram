...
fn real_unix_peer_uids_have_independent_source_buckets_and_logs() -> Result<(), Error> {
    ...
    let helper_root = tmpdir();
    std::fs::set_permissions(helper_root.path(), std::fs::Permissions::from_mode(0o755))?;
    let helper_executable = helper_root.path().join("bind-peer-helper");
    std::fs::copy(env::current_exe()?, &helper_executable)?;
    std::fs::set_permissions(&helper_executable, std::fs::Permissions::from_mode(0o755))?;
    // GitHub 的工作区可能位于只允许 runner 用户遍历的目录中。降权后的辅助进程不能直接
    // exec 当前测试路径，因此把相同二进制复制到 root 所有、不可由客户端 UID 修改且可遍历
    // 的临时目录。权限在降权前固定，避免为了测试而放宽工作区本身。
    // GitHub may place the workspace below directories traversable only by the runner user. A
    // deprivileged helper therefore cannot reliably exec the current test path. Copy the same
    // binary into a root-owned, client-immutable, traversable temporary directory instead of
    // weakening workspace permissions.
    let mut cmd = ram_command(root.path(), port());
    cmd.args([
        "--auth",
        TEST_AUTH_RULE,
    ...
    let mut first_command = unix_peer_helper_command(
        &helper_executable,
        &socket,
        "hold",
        Some(&marker),
    ...
    let second = unix_peer_helper_command(
        &helper_executable,
        &socket,
        "read",
        None,
