...
- Release workflow 使用 environment protection、最小 `contents: write` 和仅用于 GitHub
  attestation 的短期 OIDC，不保存长期发布 token，也不上传外部软件包注册表。
- 保持 build provenance、语义验证后的 SBOM attestation、archive/binary checksum 和最终 tar
  原生架构烟测；所有附件先进入草稿，完整后才公开 Release。
- GitHub Release 只能由独立 finalizer 公开；finalizer 必须重新下载已验证制品，并复核草稿
  身份以及附件数量、名称、大小与 SHA-256。
...
- 每季度核对 organization/repository owner、outside collaborator、Actions secret、environment
  reviewer、deploy key 和 GitHub App；立即移除离任或闲置访问。
...
- [ ] 发布 environment、attestation OIDC 和最小权限已复核。
...
- Protect the release environment, grant only `contents: write`, and use short-lived OIDC solely for
  GitHub attestations. Store no long-lived publishing token and upload to no external package registry.
- Preserve build provenance, semantically checked SBOM attestations, archive/binary checksums, and
  native-architecture smoke tests. Upload everything to a draft before publication.
- Only the independent finalizer may publish a GitHub Release. It redownloads the verified artifacts
  and rechecks draft identity plus every attachment's count, name, size, and SHA-256. A retry may
  delete only a draft carrying the exact repository/tag/commit marker; published
...
- Quarterly audit organization/repository owners, outside collaborators, Actions secrets, environment
  reviewers, deploy keys, and GitHub Apps; remove stale access immediately.
...
- [ ] Release environment, attestation OIDC, and least privilege are reviewed.
