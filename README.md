...
`TextDecoder`、`padStart()`、`classList` 和现代 DOM API。
...
覆盖率使用与常规测试隔离的默认 TLS 生产配置插桩构建；`--no-default-features` 由普通
x86_64/ARM64 矩阵完整测试。短生命周期子进程按二进制签名在线归并，随后仓库脚本逐个验证
raw profile，并以每次最多两个已验证输入的平衡树生成最终索引。任何无效 profile 都会使
任务失败并保留原始输入，CI 会上传这些 raw profile 供诊断；`fuzzing` 仅用于 fuzz harness，
不计入常规覆盖率：

```sh
cargo install --locked cargo-llvm-cov --version 0.8.7
cargo llvm-cov clean --workspace
export LLVM_PROFILE_FILE_NAME='ram-%m.profraw'
cargo llvm-cov --no-report --all-targets --locked --remap-path-prefix
host="$(rustc -vV | sed -n 's/^host: //p')"
python3 scripts/merge-coverage-profiles.py \
  --llvm-profdata "$(rustc --print sysroot)/lib/rustlib/$host/bin/llvm-profdata" \
...
失败。由于 Rust 插桩口径已固定为默认 TLS 生产配置，旧 Rust 基线已清除；至少积累 10 次
当前口径下稳定的 main 运行后，维护者才可在本地使用
...
Coverage uses an isolated default-TLS production instrumentation build; the regular x86_64/ARM64
matrix fully tests the no-default-features build. `scripts/merge-coverage-profiles.py` validates raw
profiles individually and builds the final index as a balanced tree, failing without deleting the
inputs if any profile is unusable; CI uploads those raw inputs for diagnosis.
`scripts/report-coverage.mjs` then groups global/frontend/auth/filesystem/write-precondition/DAV/Range
totals. Harness-only fuzzing is not regular coverage. The obsolete Rust baseline was cleared when
the instrumentation scope was fixed; `tests/coverage/policy.json` remains trend-only until at least
ten stable main runs under the current scope. Only a reviewed local `--update-baseline` may set
floors/enforce.
