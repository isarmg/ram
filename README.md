...
- 前端静态校验：Node.js `24`、ESLint `10`、TypeScript `7`。
...
1. 命令行参数。
2. `RAM_*` 环境变量。
3. 可执行文件同目录的 `config.yaml`（仅弃用兼容期）。
4. 内置默认值。

以上顺序适用于同一个配置项。`allow-all` 与具体 `allow-*` 是总项/具体项关系，按下述
专门顺序合并；这意味着环境中的具体例外会优先于 CLI 的总开关。

`--config <path>` 或 `RAM_CONFIG=<path>` 显式选择配置文件；命令行仍高于环境变量。
若两者均未设置，Ram 才会兼容性地查找“当前可执行文件所在目录”中的
`config.yaml`。只要环境中**存在** `RAM_NO_CONFIG`（包括空字符串、`0` 或 `false`），
就会关闭这一自动发现；它不会忽略显式 `--config`/`RAM_CONFIG`。因此不要把
`RAM_NO_CONFIG=false` 当作“重新启用”。显式路径不存在或不安全会直接启动失败。
配置文件使用 kebab-case，未知字段会导致启动失败，避免拼错安全开关后静默运行。

可执行文件相邻 `config.yaml` 只是一项待移除的旧版兼容行为，迁移时间表如下：

- `v0.47.x`–`v0.49.x`：继续自动发现，兼容窗口从弃用公告发布起不少于 90 天；实际加载
  相邻配置时会在 stderr 输出一次弃用警告。显式配置以及设置了 `RAM_NO_CONFIG` 的启动
  不输出该警告。
- `v0.50.x`：默认不再自动发现；仅可显式设置 `RAM_LEGACY_AUTO_CONFIG=1` 临时恢复，
  该逃生门只保留这一代。若同时存在 `RAM_NO_CONFIG`，仍以禁用为准。
- `v0.51.0`：删除相邻配置发现和 `RAM_LEGACY_AUTO_CONFIG`；部署必须使用 `--config` 或
  `RAM_CONFIG`。

生产服务现在就应改用显式绝对路径，不要等到 `v0.50`。打包脚本也不应通过把
`config.yaml` 复制到二进制目录来隐式改变服务配置。
...
`ram --check-config --config /etc/ram/config.yaml` 后，再用同一个显式路径启动。不要把新部署
放到二进制旁的弃用兼容路径。下面展示常用字段的完整组合：
...
显式使用 `/usr/local/bin/ram --config /etc/ram/config.yaml`，不要依赖二进制旁的兼容
自动发现。账号规则和 token 密钥通过 systemd credential 注入，避免进入 argv、环境变量
或长期可见的普通配置文件。
...
| `server/write.rs` | PUT/PATCH/DELETE/MKCOL/COPY/MOVE |
| `server/webdav.rs` | PROPFIND/PROPPATCH 和 DAV 能力 |
| `server/filesystem.rs` | 根目录能力、受限打开和原子文件操作 |
...
`.github/coverage-baseline.json` 处于 `trend` 模式：目标用于显示差距，百分比暂不让构建
...
cargo +nightly-2026-07-20 check --manifest-path fuzz/Cargo.toml --locked
...
for target in "${targets[@]}"; do
  ...
  cargo +nightly-2026-07-20 fuzz run "$target" "$corpus_dir" -- \
...
1. 使用 Rust 1.96 对 Rust 2024 源码执行 fmt、Clippy、双特性组合测试和文档检查。
...
压缩包包含 `ram`、双许可证、本文档、配置示例、依赖清单、运行时链接信息和供应链
...
- Apache License 2.0，见 `LICENSE-APACHE`。
- MIT License，见 `LICENSE-MIT`。

使用者可以按照任一许可证的条款使用、修改和分发本项目。
...
- Rust edition 2024, Rust 1.96.0, Cargo resolver 3.
- Native ES modules/ESNext frontend, without transpilation, polyfills, or a legacy bundle.
- Development checks use Node.js 24, ESLint 10, and TypeScript 7.
...
Ordinary fields merge in this order: CLI, `RAM_*` environment, adjacent legacy `config.yaml`, then
defaults. Explicit `--config <path>` or `RAM_CONFIG=<path>` selects YAML, with CLI winning. Adjacent
auto-discovery occurs only when neither is set and is being removed:

- v0.47.x-v0.49.x: discover with a deprecation warning, for at least a 90-day migration window;
- v0.50.x: disabled by default, temporarily restored only by `RAM_LEGACY_AUTO_CONFIG=1`;
- v0.51.0: discovery and the escape hatch are removed.

The mere presence of `RAM_NO_CONFIG`—including empty, `0`, or `false`—disables discovery but never an
explicit path. Production should use an explicit absolute path now. YAML rejects unknown fields.
...
placeholder password, run `--check-config`, and start with the same explicit path. That file is a safe
template: loopback-only, privileged operations off, and deliberately not runnable until credentials
are replaced.
...
| `server/write.rs`, `filesystem.rs` | Mutations, root capability, atomic/durable operations |
| `server/webdav.rs`, `dav_routes.rs` | Bounded DAV subset and dispatch |
...
filesystem/write-precondition/DAV/Range totals. `.github/coverage-baseline.json` remains trend-only
...
Archives contain `ram`, both licenses, README, example configuration, dependency/licenses, runtime
...
Ram is dual-licensed under Apache License 2.0 (`LICENSE-APACHE`) or MIT (`LICENSE-MIT`). Users may use,
modify, and distribute it under either license's terms.
